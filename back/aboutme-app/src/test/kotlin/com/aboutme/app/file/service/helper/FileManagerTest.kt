...
class FileManagerTest : DescribeSpec({
    ...
    describe("파일 대체") {
        context("올바른 파일 경로를 제공하면") {
            ...
            FileManager.replaceFile(filePath, newFile)
            it("파일이 대체된다") {
                Files.readAllBytes(filePath) shouldBe newFile.bytes
            }
        }

        context("디렉토리 경로를 제공하면") {
            Files.createDirectories(filePath.parent)
            it("예외가 발생한다") {
                shouldThrow<IllegalArgumentException> {
                    FileManager.replaceFile(filePath.parent, mockFile)
                }
            }
        }

        context("잘못된 파일 경로를 제공하면") {
            it("예외가 발생한다") {
                val invalidPath = basePath.resolve("invalid-file.txt")
                shouldThrow<IllegalArgumentException> {
                    FileManager.replaceFile(invalidPath, mockFile)
                }
            }
        }
    }
